export function injectLaunchRipple(mascotUrl: string): void {
  ...
  gl.shaderSource(fs, [
    ...
    "  vec2 pos=(uv-0.5)*vec2(aspect,1.0);float dist=length(pos);",
    "  float maxR=1.2;float alpha=0.0;vec3 col=vec3(0.0);",
    "  vec3 colors[4];",
    "  colors[0]=vec3(0.26,0.52,0.96);colors[1]=vec3(0.92,0.26,0.21);",
    "  colors[2]=vec3(0.98,0.74,0.02);colors[3]=vec3(0.20,0.66,0.33);",
    "  for(int i=0;i<4;i++){",
    "    float fi=float(i);float delay=fi*0.08;",
    "    float lT=clamp((u_t-delay)/(1.0-delay),0.0,1.0);",
    "    float lE=lT*lT*(3.0-2.0*lT);",
    "    float ringR=lE*maxR;float thick=0.012+0.008*(1.0-lE);",
    "    float rd=abs(dist-ringR);",
    "    float ring=smoothstep(thick,thick*0.2,rd);",
    "    float fade=(1.0-lE)*(1.0-smoothstep(0.7,1.0,lT));",
    "    ring*=fade;ring+=exp(-rd*40.0)*0.3*fade;",
    "    ring*=sin(dist*80.0-u_t*12.0+fi*1.5)*0.1+0.9;",
    "    col+=colors[i]*ring;alpha+=ring;",
    "  }",
    "  float cg=exp(-dist*dist*20.0)*(1.0-u_t*u_t*(3.0-2.0*u_t))*0.4;",
    "  col+=vec3(0.26,0.52,0.96)*cg;alpha+=cg;",
    "  alpha*=1.0-smoothstep(0.65,1.0,u_t);",
